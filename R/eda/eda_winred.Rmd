...
```{r}
df_ls$winred %>% filter(is.na(url)) %>% nrow()
```

```{r}
df_ls$winred %>% filter(url == "") %>% nrow()
```
...
```{r}
setdiff(
  df_ls$winred %>% filter(is.na(url)) %>% .$name, 
  df_ls$winred %>% filter(!is.na(url)) %>% .$name
)
```
...
`r round(length(unique(temp$name)) / length(unique(df_ls$winred$name)) * 100, digits = 1)`% (`r length(unique(temp$name))` out of `r length(unique(df_ls$winred$name))` entities) change URL link at some point in time. But most of these seem to be because of addition of "?sc=winred-directory". 
...
`r round(length(unique(temp$name)) / length(unique(df_ls$winred$name)) * 100, digits = 1)`% (`r length(unique(temp$name))` out of `r length(unique(df_ls$winred$name))` entities) change URL link at some point in time.
